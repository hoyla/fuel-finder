-- Distinguish genuinely Independent brands from unmapped ones.
--
-- Previously the COALESCE fallback in the current_prices view used
-- 'Independent' for any brand without a brand_categories entry, making
-- it impossible to tell whether a brand was explicitly categorised as
-- Independent or simply hadn't been reviewed yet.
--
-- Now: unmapped brands get 'Uncategorised', while 'Independent' is
-- reserved for brands explicitly assigned that category.

-- Rebuild the materialized view with the new fallback
