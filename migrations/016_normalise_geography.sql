-- Normalise geographic casing and add postcode-based country resolution.
--
-- Strategy: derive country from the postcode_regions table (which maps
-- every UK postcode prefix to a region). This provides a general fallback
-- when postcodes.io data is missing, without hardcoding specific country
-- name variants from the GOV.UK API.
--
-- Fallback chain: postcodes.io country → postcode_regions country → 'Other/Unknown'

-- 1. Fix region casing to match postcodes.io
UPDATE postcode_regions
SET region = 'Yorkshire and The Humber'
WHERE region = 'Yorkshire and the Humber';

-- 2. Add country column to postcode_regions, derived from region
ALTER TABLE postcode_regions ADD COLUMN IF NOT EXISTS country TEXT;

UPDATE postcode_regions SET country = CASE
    WHEN region = 'Northern Ireland' THEN 'Northern Ireland'
    WHEN region = 'Scotland' THEN 'Scotland'
    WHEN region IN ('Wales', 'South Wales', 'North Wales') THEN 'Wales'
    ELSE 'England'
END;

-- Rebuild the materialized view with the country normalisation
