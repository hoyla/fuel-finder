-- Rebuild current_prices materialized view to incorporate price corrections.
--
-- When a correction exists for a fuel_price record, the corrected_price is
-- used instead of the original price.  Additionally, the anomaly_flags on
-- the original record are ignored when a correction is present (the user
-- has reviewed and fixed the value), but the IQR statistical outlier check
-- still applies to the corrected value as a safety net.
